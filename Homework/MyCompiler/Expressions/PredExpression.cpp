...
PredExpression::PredExpression(Expression *prime_expr){
	_prime_expr = prime_expr;
}


ExpressionResult PredExpression::emit(){
	ExpressionResult prime_result = _prime_expr->emit();

	type = _prime_expr->type;
	if (type != int_type && type != bool_type && type != char_type){
		throw "Cannot call pred on something besides an int, a bool, or a char";
	}	
	...
	if(type == int_type || type == char_type){
		std::cout << "\taddi\t" << ret_reg << ", " << expr_reg << ", -1\t\t#Pred expression" << std::endl;
